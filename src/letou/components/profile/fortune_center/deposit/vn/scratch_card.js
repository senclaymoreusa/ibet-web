...
import { authCheckState, sendingLog, AUTH_RESULT_FAIL, postLogout, logout } from '../../../../../../actions';
...
class ScratchCard extends Component {
    ...
    componentWillReceiveProps(props) {
        ...
        axios.get(API_URL + 'users/api/user/', config)
            .then(res => {
                ...
                this.setState({ isFavorite: res.data.favorite_payment_method === 'scratchcard' });
            });
    }
    ...
    render() {
        ...
        const { amount, cardNumber, serialNumber, provider, isFavorite } = this.state;
                ...
                <Grid container spacing={2} className={classes.contentGrid}>
                    ...
                    <Grid item xs={12}>
                        <Select
                            ...
                            input={<BootstrapInput name="provider" id="provider-select" />}>
                            ...
                            </MenuItem>
                            {
                                providers.map(p => (
                                    <MenuItem key={p.value} value={p.value} >
                                        <div style={{ width: 100 }}>
                                            <img src={images.src + p.img} alt="" width="80"/>
                                        </div>
                                        <span className={classes.info}>{p.label}</span>
                                    </MenuItem>
                                ))
                            }
                        </Select>
                    ...
                    <Grid item xs={12}>
                        <Select
                            ...
                            input={<BootstrapInput name="amount" id="amount-select" />}>
                            <MenuItem key='none' value='none' disabled>
                    ...
                    <Grid item xs={12} className={classes.detailRow}>
                        <TextField
                            ...
                            value={cardNumber}
                            error={this.state.cardNumberFocused && cardNumber.length === 0}
                            helperText={(this.state.cardNumberFocused && cardNumber.length === 0) ? this.getLabel('invalid-card-number') : ' '}
                            InputProps={{
                    ...
                    <Grid item xs={12} className={classes.detailRow} style={{ marginBottom: 30 }}>
                        <TextField
                            ...
                            value={serialNumber}
                            error={this.state.serialNumberFocused && serialNumber.length === 0}
                            helperText={(this.state.serialNumberFocused && serialNumber.length === 0) ? this.getLabel('invalid-serial-number') : ' '}
                            InputProps={{
                                disableUnderline: true,
                                endAdornment: (
                                    <InputAdornment position="end" >
                    ...
                    <Grid item xs={12} className={classes.detailRow}>
                        <span className={classes.info}>{this.getLabel('fgo-wrong')}</span>
                    </Grid>
                </Grid>
            </div>
        )
...
export default withStyles(styles)(withRouter(injectIntl(connect(mapStateToProps, { authCheckState, logout })(ScratchCard))));