...
class Phone_Verification extends React.Component {
  ...
  async handle_code_1(event) {
    if (
      !event.target.value ||
      (event.target.value.match(/^[0-9]+$/) && event.target.value.length < 2)
    ) {
  ...
  check_valid() {
    if (
      ...
      axios
        .post(API_URL + 'users/api/verifyactivationcode/', {
          username: this.props.signup_username,
          code:
            this.state.code_1.toString() +
            this.state.code_2.toString() +
            this.state.code_3.toString() +
            this.state.code_4.toString()
        })
        .then(res => {
          ...
          if (this.props.refer_id) {
            axios.get(
              API_URL +
                `users/api/referral/?referral_id=${this.props.refer_id}&referred=${this.props.signup_username}`
            );
          }
    ...
    return (
      <div style={{ backgroundColor: 'white', height: 640, width: 770 }}>
        <div className="signup-title">
          <div style={{ paddingTop: 20 }}>VERIFICATION</div>
          ...
          <img
            ...
            onClick={() => {
              this.props.hide_phone_verification();
              axios.post(API_URL + 'users/api/cancelregistration/', {
                username: this.props.signup_username
              });
            }}
          />
        ...
        <div style={{ fontSize: 38, fontWeight: 'bold', marginLeft: 77 }}>
          {' '}
          We sent you a code to{' '}
        </div>
        <div style={{ fontSize: 38, fontWeight: 'bold', marginLeft: 77 }}>
          {' '}
          confrim you phone number{' '}
        </div>
              ...
              {this.state.error && (
                <div style={{ position: 'absolute', left: 400, top: 292 }}>
                  <div style={{ color: 'red', fontSize: 16 }}>
                    <FormattedMessage
                      id="signup.signupphoneerror1"
                      defaultMessage="Sorry, that’s not the code we’re looking for."
                    />
                  </div>
                  <div style={{ color: 'red', fontSize: 16 }}>
                    <FormattedMessage
                      id="signup.signupphoneerror2"
                      defaultMessage="Please check and try again."
                    />
                  </div>
                </div>
              )}
        ...
        <div
          onClick={() => {
            ...
            axios
              .post(API_URL + 'users/api/generateactivationcode/', {
                username: this.props.signup_username
              })
              .then(res => {
